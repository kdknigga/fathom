...
def prepare_line_chart(
    ...
    # Collect all monthly data for scale computation
    ...
    for name, _cost in sorted_options:
        ...
        # Handle sparse data (cash options)
        ...
        lines.append(
            {
                ...
                "color": COLORS[i % len(COLORS)],
                "dash_pattern": DASH_PATTERNS[i % len(DASH_PATTERNS)],
                "pattern_id": LINE_PATTERNS[i % len(LINE_PATTERNS)],
    ...
    for j in range(num_y_labels):
        ...
        y_labels.append(
            {
                "value": _format_cost(val),
                "y": _to_float(scale_y(val)),
            }
        )
